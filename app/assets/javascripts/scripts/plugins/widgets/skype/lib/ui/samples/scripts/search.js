  ...
  $("#btn-search").click(function () {
    ...
    pSearch.getMore().then(function (results) {
      ...
      results.forEach(function (r) {
        ...
        avatar = $("<div>").addClass("photo-c").append(
            $("<img>").addClass("photo").attr("src", r.result.avatarUrl()));
        detail = $("<div>").addClass("detail").append(
            $("<div>").addClass("display-name").text(r.result.displayName()));
        tag = $("<div>").addClass("persona").addClass("persona-small").
            append(avatar).append(detail);
        $('#results').append(tag);
      });
