...
$(function () {
  ...
  $('#btn-get-contact').click(function () {
    ...
    pSearch.getMore().then(function () {
      ...
      var cNote = $("<div>");
      var noteContainer = $("<div>").addClass("note-c").append(cNote).
      ...
      var avatar = $("<div>").addClass("photo-c").append($("<img>")).
          append(avatarPresence);
      var detail = $("<div>").addClass("detail").append(cDisplayName).
          append(cTitle).append(cLocation);
      var persona = $("<div>").addClass("persona").addClass("persona-xl").
          append(noteContainer).append(avatar).append(detail);
      contact.avatarUrl.get().then(function (avatarUrl) {
        $("img", avatar).attr('src', avatarUrl).error(setDefaultAvatar);
      });
      ...
      var onPresenceChanged = function (status) {
        avatarPresence.removeClass(presenceClass).
            addClass(presenceClass = 'photo-presence-' + status);
      };
      var subP = [], subM = [];
      // display static data of the contact
      $('#result').empty().append(persona).append(cCapabilities);
      ...
      $('#subscribe2').click(function () {
        // tell the contact to notify us whenever its available capabilities
        // change
