...
registeredListeners = [];
/**
 * This web app demonstrates group and person management functionalities, such
 * as:
...
$(function () {
  'use strict';
  var personsFound, groupsFound, personsInGroup, groupsInClient, oldName; // group
                                                                          // name
                                                                          // before
                                                                          // renaming
  ...
  function doCreate(groupName) {
    ...
    g.name(groupName);
    client.personsAndGroupsManager.all.groups.add(g).
        then(createGroupSuccess, createGroupFail);
  }
  ...
  function doAddDG(group) {
    client.personsAndGroupsManager.all.groups.add(group).
        then(addDGSuccess, addDGFail);
  }
  ...
  function doRemoveGroup(group) {
    client.personsAndGroupsManager.all.groups.remove(group).
        then(removeGroupSuccess, removeGroupFail);
  }
  ...
  $('#renameGroup').click(function () {
    var group = groupsInClient[$('#groupList').val()], name = $(
        '#newGroupName').text();
    oldName = group.name();
  ...
  $('#addPerson').click(function () {
    ...
    group.persons.add(personsFound[$('#personsFoundList').val()]).
        then(addPersonSuccess, addPersonFail);
  });
  // -----------------------------------------------------
  ...
  $('#removePerson').click(function () {
    var groupName = $('#groupList').val(), person = personsInGroup[$(
        '#personList').val()];
    doRemovePerson(groupName, person);
  });
  // when the user clicks on the 'Remove from all Groups' button
  $('#removeFromAll').click(function () {
    var i, g, person = personsInGroup[$('#personList').
        val()], gs = client.personsAndGroupsManager.all.groups();
    for (i = 0; i < gs.length; ++i) {
      g = gs[i];
      g.persons.subscribe();
      g.persons.removed(removeFromPersonList);
    }
    client.personsAndGroupsManager.all.persons.remove(person).
        then(removePersonSuccess, removePersonFail);
  });
  // -----------------------------------------------------
    ...
    if (v) {
      ...
      pSearch.getMore().then(function (results) {
        ...
        for (i = 0; i < results.length; ++i) {
          ...
          $('#personsFoundList').
              append($('<option></option>').text(person.id()));
        }
      });
