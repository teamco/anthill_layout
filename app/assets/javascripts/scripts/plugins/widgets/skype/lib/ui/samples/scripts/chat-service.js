...
$(function () {
  ...
  var addedListener = client.conversationsManager.conversations.added(
      function (conversation) {
        ...
        chatService.accept.enabled.when(true, function () {
          // instead of using chatService.accept.enabled.changed,
          // selfParticipant.chat.state.changed should also work.
          ...
          if (fAccept) {
            ...
            uiToChatState();
            $(".chat-name").
                text(conversation.participants(0).person.displayName());
          }
          else {
            chatService.reject();
          }
  ...
  var removedListener = client.conversationsManager.conversations.removed(
      function (conversation) {
  ...
  function startInstantMessaging() {
    ...
    pSearch.getMore().then(function () {
      ...
      chatService = conversation.chatService;
      conversation.selfParticipant.chat.state.when("Connected",
          function (state) {
