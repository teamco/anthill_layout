...
registeredListeners = [];
/**
 * This script demonstrates how to use audio/video along with messaging in an
...
$(function () {
  ...
  var addedListener = client.conversationsManager.conversations.added(
      function (conversation) {
            ...
            if (newState == 'Connected') {
              if (conversation.participants.size() == 1) {
                p.video.channels(0).stream.source.sink.
                    container(document.getElementById("render-p-window"));
              }
              else {
                var partcipant = conversation.participants(0);
                partcipant.video.channels(0).stream.source.sink.
                    container(document.getElementById("render-p-window"));
                partcipant.video.channels(0).isStarted.set(true);
                p.video.channels(0).stream.source.sink.
                    container($(".add-video-container")[0]);
        ...
        function onAudioVideoNotified() {
          // AV invitation may come from a 1:1 conversation only, so the caller
          // is the single participant in the participants collection
          ...
          if (selfParticipant.video.state() == 'Notified') {
            if (confirm('Accept a video call from ' + name + '?')) {
              ...
              // accept the incominng video call or after it is accepted or
              // even  later, when the selfParticipant video state becomes
              // "Connected"
              dfdVideoAccept = videoService.accept();
            }
            else if (confirm(
                    'Accept a video call from ' + name + ' with audio only?\n' +
        ...
        ;
        selfParticipant.audio.state.changed(
            function (newState, reason, oldState) {
        ...
        selfParticipant.video.state.changed(
            function (newState, reason, oldState) {
              var selfChannel;
              if (newState == 'Notified' && !timerId) {
                timerId = setTimeout(onAudioVideoNotified, 0);
                ...
                selfChannel.stream.source.sink.container.set(
                    document.getElementById("render-self-window"));
              }
            });
  ...
  $("#btn-video").click(function () {
    ...
    if (client.conversationsManager.conversations.size() == 0) {
      findPerson(uri).then(function (person) {
        conv = client.conversationsManager.getConversation(person);
        // video stream containers can be set either before calling start() or
        // after that async call is resolved or when participant video state
        // becomes "Connected".
      ...
      // note that if audio service is already started this will just start
      // video
      conv = client.conversationsManager.conversations(0);
      if (conv) {
        // video stream containers can be set either before calling start() or
  ...
  $('#btn-show-video, #btn-hide-video').click(function () {
    var conv = client.conversationsManager.conversations(
        0), val, vcSelf, dfd, audio;
    if (conv) {
      audio = conv.selfParticipant.audio;
    ...
    if (evt.keyCode == 13) {
      ...
      if (!hasIM) {
        ...
        conv.historyService.activityItems.added(function (message) {
          ...
          $(".message-list").
              animate({scrollTop: $(".message-list")[0].scrollHeight}, "fast");
        });
        hasIM = true;
      }
      else {
  ...
  $("#btn-add-participant").click(function () {
    var conv = client.conversationsManager.conversations(0), uri = $(
        '#av-add-p').text(), dfd;
    if (conv) {
