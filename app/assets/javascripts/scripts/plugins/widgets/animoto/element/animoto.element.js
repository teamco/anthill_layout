    ...
    return AnimotoElement.extend('AnimotoElement', {
        ...
        renderEmbeddedContent: function renderEmbeddedContent(opts) {
            ...
            var $embed = opts.type === 'object' ?
                [
                    '<', opts.type, ' id="', opts.id, '" width="100%" height="100%" classid="', opts.classid, '">',
                    '<param name="movie" value="', opts.movie, '"/>',
                    '<param name="allowFullScreen" value="true"/>',
                    '<param name="allowscriptaccess" value="always"/>',
                    '<embed id="', opts.id, '" src="', opts.movie, '" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="100%" height="100%"></embed>',
                    '</', opts.type, '>'
                ].join('') :
