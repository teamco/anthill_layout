...
var Qrcodesvg = (function () {
  ...
  return function (input, parentId, size, options) {
    ...
    this.init = function () {
      if (Raphael ==
          undefined) {
        throw new Error("Raphael object not found");
      }
     ...
     * @param object attributes            object passed to Raphael's
     *     Element.attr() method
     */
    this.draw = function (settings, attributes) {
      ...
      for (; i < patterns.length; i++) {
        ...
        for (j = 0; j < patterns[i].length; j++) {
          ...
          Qrcodesvg.drawMethods[settings["method"]](svg, settings,
              new_attributes);
        }
      }
    };
...
Qrcodesvg.encoder = (function () {
  ...
  return function (instring, ecclevel) {
    ...
    var strinbuf = [], eccbuf = [], qrframe = [], framask = [], rlens = [], // data
                                                                        // input
                                                                        // and
                                                                        // ecc
                                                                        // append,
                                                                        ...
                                                                        // fixed
                                                                        // part
                                                                        // of image, run lengths for badness
        version, width, neccblk1, neccblk2, datablkw, eccblkwid, // Control
                                                                 // values -
                                                                 // width is
                                                                 // based on
                                                                 // version,
    ...
    // set bit to indicate cell in qrframe is immutable.  symmetric around
    // diagonal
    ...
    // enter alignment pattern - black to qrframe, white to mask (later black
    // frame merged to mask)
    ...
    this.appendrs = function (data, dlen, ecbuf, eclen) {
      ...
      for (i = 0; i < dlen; i++) {
        ...
        {
          for (j = 1; j < eclen; j++) {
            strinbuf[ecbuf + j - 1] = strinbuf[ecbuf + j] ^
                this._get_constant("gexp")[this.modnn(fb + genpoly[eclen - j])];
          }
        } else {
        ...
        strinbuf[ecbuf + eclen - 1] = fb == 255 ? 0 :
            this._get_constant("gexp")[this.modnn(fb + genpoly[0])];
      }
    };
    ...
    // Calculate how bad the masked image is - blocks, imbalance, runs, or
    // finders.
      ...
      for (y = 0; y < width - 1; y++) {
        for (x = 0; x < width - 1; x++) {
          if ((qrframe[x + width * y] && qrframe[(x + 1) + width * y]
              ...
              || qrframe[x + width * (y + 1)] ||
              qrframe[(x + 1) + width * (y + 1)])) // all white
          {
            thisbad += N2;
          }
        }
    ...
    this.genframe = function () {
      ...
      for (i = 0; i < eccblkwid; i++) {
        ...
        for (j = i; j > 0; j--) {
          genpoly[j] = genpoly[j]
              ? genpoly[j - 1] ^ this._get_constant("gexp")[this.modnn(
                  this._get_constant("glog")[genpoly[j]] + i)] : genpoly[j - 1];
        }
        genpoly[0] = this._get_constant("gexp")[this.modnn(
            this._get_constant("glog")[genpoly[0]] + i)];
      }
      for (i = 0; i <= eccblkwid; i++) {
        genpoly[i] = this._get_constant("glog")[genpoly[i]];
...
Qrcodesvg.drawMethods = {
  ...
  round: function (svg, settings, attributes) {
    ...
    /* bottom right */
    path +=
        (x + width) + "," + ((corners[2]) ? y + height - radius : y + height);
    path += (!corners[2]) ? " L " :
