...
define(function definePageRules() {

  return require(['lib/packages/go'], function () {
    ...
    return myDiagram;
    function init() {
      if (window.goSamples) goSamples();  // init for these samples -- you don't
                                          // need to call this
          ...
          $(go.Node, 'Auto',
              {selectionAdorned: false},
              {
                mouseDrop: function (e, n) {
                  // when the selection is entirely ports and is dropped onto a
                  // Group, transfer membership
              ...
              $(go.Shape,
                  {
                    ...
                    spot1: new go.Spot(0, 0, 5, 1),  // keep the text inside the
                                                     // shape
                  ...
                  $(go.Panel, 'Spot',
                      {
                        name: 'ICON',
                        height: 60
                      },  // an initial height; size will be set by
                      // InputOutputGroupLayout
                      $(go.Shape,
                          {
                            fill: null,
                            stroke: null,
    ...
    // For each data object in the "ins" Array of the node data, add a "port"
    // Node to the Group. For each data object in the "outs" Array, add a "port"
    // Node to the Group. For each link data, convert the "from" and "fromPort"
    // information to the actual "port" Node, and then the same for "to" and
    // "toPort". The internal model uses property names starting with "_" to
    // avoid having Model.toJson() write them out.
    function setupDiagram(nodes, links) {
      ...
      // then create all of the ports, implemented as Nodes that are members of
      // those Groups
      for (var i = 0; i < nodes.length; i++) {
        var nodedata = nodes[i];
      ...
      // now Groups and Nodes exist, so can find the Node corresponding to a
      // node's port

      // finally process all of the links, to account for ports actually being
      // member nodes
      for (var i = 0; i < links.length; i++) {
        ...
        if (fromNode !== null && toNode !== null) {
          // look in the Group for a "port" Node with the right name and
          // directionality
          ...
          if (fromPortNode !== null && toPortNode !== null) {
            ...
            linkdata.from =
                linkdata.fromPort = linkdata.to = linkdata.toPort = undefined;
          }
        }
      }
    ...
    function save() {
      // can't just call myDiagram.model.toJson() -- need to transform to
      // external format
    ...
    InputOutputGroupLayout.prototype.doLayout = function (coll) {
      ...
      if (this.group !== null && this.group.location.isReal()) loc =
          this.group.location;
      ...
      coll.each(function (n) {
        ...
        n.position =
            new go.Point(loc.x + iconAreaWidth / 2 + rightwidth - n.actualBounds.width, y);
        y += n.actualBounds.height + portSpacing;
      });

      // then position the group and size its icon area
      if (this.group !== null) {
        // position the group so that its ICON is in the middle, between the
        // "ports"
        this.group.location = loc;
        // size the ICON so that it's wide enough to overlap the "ports" and
        // tall enough to hold all of the "ports"
        var icon = this.group.findObject('ICON');
        if (icon !== null) icon.desiredSize =
            new go.Size(iconAreaWidth + leftwidth / 2 + rightwidth / 2, Math.max(leftheight, rightheight) + 10);
      }
    };
  });
});
